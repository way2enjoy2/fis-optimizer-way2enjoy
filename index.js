...
function Way2enjoy() {
	var self = this;
	if (!(this instanceof Way2enjoy)) {
		return new Way2enjoy();
	}
...
Way2enjoy.prototype.src = function(file) {
	var self = this;
	if (!arguments.length) {
		return self._src;
	}
...
Way2enjoy.prototype.dest = function(dir) {
	var self = this;
	if (!arguments.length) {
		return self._dest;
	}
...
Way2enjoy.prototype.key = function(apiKey) {
	var self = this;
	if (!arguments.length) {
		return self._key;
	}
...
Way2enjoy.prototype.run = function() {
	var self = this;
	//获取存放所有图片路径的数组
	var src = self.src();
	...
	if (imgList.length == 0) {
		...
		imgList.forEach(function(file) {
			...
			input.pipe(request.post('https://way2enjoy.com/modules/compress-png/way2enjoy-cli2.php', {
				auth: {
					'user': 'api',
					'pass': key
				}
...
//获取文件夹内所有图片路径数组
Way2enjoy.prototype.getImg = function(src) {
	var self = this;
	var imgList = [];
...
//判断是否是图片
Way2enjoy.prototype.checkImg = function(file) {
	var imgType = ['.png', '.jpg', '.gif'];
...
Way2enjoy.prototype.equiLong = function(str, len) {
